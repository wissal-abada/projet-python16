...
def signup_view(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            data.save()
            messages.success(request, "Membre, bienvenue !")
            return HttpResponseRedirect('/user')
        else:
            messages.warning(request, "Ce nom d'utilisateur ou cette adresse électronique est utilisé(e) !")
...
def menu(request, id):
    try:
        ...
        messages.warning(request, "Aucun contenu trouvé !")
        link = '/error'
        return HttpResponseRedirect(link)
